...
## TRIGGER
This skill activates when a user arrives via a survey link generated by the bot, OR when any user message contains phrases like:
- "take the survey"
- "firm survey"
- "tech stack survey"
- "software survey"
...
When this skill activates, generate a unique survey session link in the format:
`[YOUR_APP_URL]/survey?id=[random-8-char-alphanumeric]`
and greet the user. If the user already arrived via a survey link, skip link generation and begin the greeting immediately.
